...
struct spm_node_subpattern {
    ...
    pattern_ty pattern;
    // TODO: Ditch this and be smarter...
    bool preserve;
};
...
spm_node_subpattern_new(struct compiler *c, spm_node_subpattern *next,
                        pattern_ty pattern, bool preserve)
{
    assert(pattern);
    spm_node_subpattern *s = _PyArena_Malloc(c->c_arena,
                                             sizeof(spm_node_subpattern));
    if (s) {
        s->next = next;
        s->pattern = pattern;
        s->preserve = preserve;
...
spm_node_pattern_new(struct compiler *c, spm_node_pattern *next,
                     match_case_ty match_case, pattern_ty pattern)
{
    ...
    n->match_case = match_case;
    n->pattern = pattern;
    n->stores = PyList_New(0);
    if (n->stores == NULL || _PyArena_AddPyObject(c->c_arena, n->stores)) {
        return 0;
    }
    n->stacksize = 1;
    n->reachable = false;
...
// Return -len if there is a star, or just len if there isn't.
static Py_ssize_t
spm_sequence_len(pattern_ty p)
{
    assert(p->kind == MatchSequence_kind);
    Py_ssize_t len = asdl_seq_LEN(p->v.MatchSequence.patterns);
    if (spm_sequence_star_index(p) < 0) {
        return len;
    }
    return -len;
}
...
spm_cleanup(struct compiler *c, spm_node_pattern *n, basicblock *end)
{
    ...
    while (n->next) {
        ...
        if (n->stacksize) {
            RETURN_IF_FALSE(pattern_helper_rotate(c, nstores + 1));
        }
        while (nstores--) {
...
spm_check(struct compiler *c, spm_node_pattern *n)
{
    ...
    if (stack_pops || name_pops) {
        ...
        for (Py_ssize_t i = 0; i < keep; i++) {
            RETURN_IF_FALSE(pattern_helper_rotate(c, keep + name_pops));
        }
        while (name_pops--) {
            ADDOP(c, POP_TOP);
        }
...
spm_store(struct compiler *c, spm_node_pattern *n, PyObject *name)
{
    if (name) {
        ...
        RETURN_IF_FALSE(pattern_helper_rotate(c, n->stacksize));
        n->stacksize--;
    }
...
spm_compile_mapping_d(struct compiler *c, spm_node_pattern *n, Py_ssize_t len)
{
    ...
    SPM_LOOP_BEGIN;
        ...
        if (p->v.MatchMapping.rest) {
            ...
            // for key in TOS:
            //     del rest[key]
            ADDOP_I(c, SWAP, 3);             // [values, keys, subject]
            ADDOP_I(c, SWAP, 2);             // [values, subject, keys]
            ADDOP_I(c, BUILD_MAP, 0);        // [values, subject, keys, empty]
            n->stacksize++;
            ADDOP_I(c, SWAP, 3);             // [values, empty, subject, keys]
            if (n->subpatterns->preserve) {
                ADDOP_I(c, COPY, 1);         // [values, empty, keys, subject, subject]
                n->stacksize++;
                RETURN_IF_FALSE(pattern_helper_rotate(c, 5));  // [subject, values, empty, keys, subject]
            }
            ...
            while (i) {
                ...
                ADDOP_I(c, SWAP, 2);         // [subject?, values, copy, keys..., copy, key]
        ...
        Py_ssize_t i = len;
        while (i--) {
            pattern_ty subpattern = asdl_seq_GET(p->v.MatchMapping.patterns, i);
            if (!spm_subpattern(c, n, subpattern, false)) {
                return 0;
            }
        }
    SPM_LOOP_END;
...
spm_compile_mapping_e(struct compiler *c, spm_node_pattern *n)
{
    SPM_LOOP_BEGIN;
        ...
        if (p->v.MatchMapping.rest) {
            if (n->subpatterns->preserve) {
                ADDOP_I(c, COPY, 1);         // [subject, subject]
                n->stacksize++;
            }
            ADDOP_I(c, BUILD_MAP, 0);        // [subject?, subject, empty]
            n->stacksize++;
            ADDOP_I(c, SWAP, 2);             // [subject?, empty, subject]
...
static int spm_compile_singleton_a(struct compiler *c, spm_node_pattern *n);
static int spm_compile_singleton_b(struct compiler *c, spm_node_pattern *n,
                                   PyObject *value);

static int
spm_compile_singleton_a(struct compiler *c, spm_node_pattern *n)
{
...
static int
spm_compile_singleton_b(struct compiler *c, spm_node_pattern *n,
                        PyObject *value)
{
    SPM_LOOP_BEGIN;
        if (n->subpatterns->preserve) {
...
static int spm_compile_value_a(struct compiler *c, spm_node_pattern *n);
static int spm_compile_value_b(struct compiler *c, spm_node_pattern *n);

static int
spm_compile_value_a(struct compiler *c, spm_node_pattern *n)
{
...
static int
spm_compile_value_b(struct compiler *c, spm_node_pattern *n)
{
    SPM_LOOP_BEGIN;
        if (n->subpatterns->preserve) {
            ADDOP_I(c, COPY, 1);
