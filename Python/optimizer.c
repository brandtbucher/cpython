...
#ifdef Py_DEBUG
                                {
                                    uint32_t next_inst = INSTR_IP(target, code) + 1 + INLINE_CACHE_ENTRIES_FOR_ITER + (oparg > 255);
    ...
    for (int i = 0; i < length; i++) {
        ...
        if (_PyUop_Flags[opcode] & (HAS_EXIT_FLAG | HAS_DEOPT_FLAG)) {
            ...
            if (jump_target != current_jump_target || current_exit_op != exit_op) {
                ...
                if (exit_op == _DEOPT) {
                    buffer[next_spare].operand = (uintptr_t)jump_target;
                }
...
)
{
    ...
    _PyUOpInstruction buffer[4] = {
        ...
        { .opcode = _EXIT_TRACE, .target = target, .format=UOP_FORMAT_TARGET }
    };
