...
_Py_Quicken(PyCodeObject *code) {
    // TODO: Maybe use counter for these sorts of checks?
    // if (code->co_quickened) {
    //     return;
    // }
    _Py_CODEUNIT *instructions = (_Py_CODEUNIT *)PyByteArray_AS_STRING(code->co_code);
    int previous_opcode = -1;
    int previous_oparg = -1;
    for (int i = 0; i < (int)(PyByteArray_GET_SIZE(code->co_code) / sizeof(_Py_CODEUNIT)); i++) {
