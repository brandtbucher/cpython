...
quicken(PyObject *co_code)
{
    ...
    for (int i = 0; i < size; i++, j++) {
        offsets[i] = j;
        j += _PyOpcode_Caches[_PyOpcode_Adaptive[_Py_OPCODE(instructions[i])]];
    }
    _Py_CODEUNIT *quickened = PyMem_Malloc(j * sizeof(_Py_CODEUNIT));
    if (!quickened) {
        PyMem_Free(offsets);
        return NULL;
    }
    // Second pass: insert adaptive/quickened/super instructions and fix jumps.
    int oparg = 0;
    int extended_args = 0;
    ...
    for (int i = 0; i < size; i++) {
        ...
        switch (opcode) {
            ...
            case LOAD_FAST:
                switch (previous_opcode) {
                    case LOAD_FAST:
                        _Py_SET_OPCODE(quickened[j - 1], LOAD_FAST__LOAD_FAST);
                        break;
                    case STORE_FAST:
                        _Py_SET_OPCODE(quickened[j - 1], STORE_FAST__LOAD_FAST);
                        break;
                    case LOAD_CONST:
                        _Py_SET_OPCODE(quickened[j - 1], LOAD_CONST__LOAD_FAST);
                        break;
                }
                break;
            case STORE_FAST:
                if (previous_opcode == STORE_FAST) {
                    _Py_SET_OPCODE(quickened[j - 1], STORE_FAST__STORE_FAST);
                }
                break;
            case LOAD_CONST:
                if (previous_opcode == LOAD_FAST) {
                    _Py_SET_OPCODE(quickened[j - 1], LOAD_FAST__LOAD_CONST);
                }
                break;
            case JUMP_BACKWARD:
        ...
        fix_jump:
            ...
            switch (extended_args) {
                case 3:
                    quickened[j - 3] = _Py_MAKECODEUNIT(EXTENDED_ARG, (oparg >> 24) & 0xFF);
                    // Fall through...
                case 2:
                    quickened[j - 2] = _Py_MAKECODEUNIT(EXTENDED_ARG, (oparg >> 16) & 0xFF);
                    // Fall through...
                case 1:
                    quickened[j - 1] = _Py_MAKECODEUNIT(EXTENDED_ARG, (oparg >> 8) & 0xFF);
                    // Fall through...
                case 0:
                    quickened[j] = _Py_MAKECODEUNIT(opcode, oparg & 0xFF);
                    break;
                default:
                    Py_UNREACHABLE();
            }
