    ...
    for (int i = 0; i < Py_SIZE(code); i++) {
        ...
        if (caches) {
            if (opcode == COMPARE_OP) {
                int next_opcode = instructions[i + caches + 1].opcode;
                if (next_opcode == POP_JUMP_IF_TRUE ||
                    next_opcode == POP_JUMP_IF_FALSE)
                {
                    int oparg = instructions[i].oparg;
                    assert((oparg >> 4) <= Py_GE);
                    int mask = compare_masks[oparg >> 4];
                    if (next_opcode == POP_JUMP_IF_FALSE) {
                        mask = mask ^ 0xf;
                    }
                    instructions[i].opcode = COMPARE_AND_BRANCH;
                    instructions[i].oparg = (oparg & 0xf0) | mask;
                }
            }
