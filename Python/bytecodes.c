...
static uint16_t invert, counter, index;
...
// BEGIN BYTECODES //
        ...
        inst(STORE_ATTR_MANAGED_DICT, (unused/1, type_version/2, index/1, value, owner --)) {
            assert(cframe.use_tracing == 0);
            PyTypeObject *tp = Py_TYPE(owner);
            assert(type_version != 0);
            DEOPT_IF(tp->tp_version_tag != type_version, STORE_ATTR);
            ...
            DEOPT_IF(!PyDict_CheckExact(dict), STORE_ATTR);
            PyHeapTypeObject *ht = (PyHeapTypeObject *)tp;
            DEOPT_IF(dict->ma_keys != ht->ht_cached_keys, STORE_ATTR);
            PyDictValues *values = dict->ma_values;
            assert(values);
            PyObject *old_value = values->values[index];
            DEOPT_IF(old_value == NULL, STORE_ATTR);
            PyObject *name = GETITEM(names, oparg);
            uint64_t new_version = _PyDict_NotifyEvent(PyDict_EVENT_MODIFIED,
                                                       dict, name, value);
            values->values[index] = value;
            Py_DECREF(old_value);
            STAT_INC(STORE_ATTR, hit);
...
family(load_attr) = {
    ...
    LOAD_ATTR_PROPERTY, LOAD_ATTR_SLOT, LOAD_ATTR_MANAGED_DICT,
