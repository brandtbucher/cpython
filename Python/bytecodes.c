...
// BEGIN BYTECODES //
        ...
        inst(LOAD_ATTR_CLASS, (unused/1, type_version/2, index/1, cls -- res2 if (oparg & 1), res)) {
            assert(cframe.use_tracing == 0);
            ...
            DEOPT_IF(entry->version != type_version, LOAD_ATTR);
        ...
        inst(LOAD_ATTR_PROPERTY, (unused/1, version/2, index/1, owner -- unused if (oparg & 1), unused)) {
            ...
            DEOPT_IF(property == NULL, LOAD_ATTR);
        ...
        inst(LOAD_ATTR_METHOD_WITH_VALUES, (unused/1, version/2, index/1, self -- res2 if (oparg & 1), res)) {
            ...
            DEOPT_IF(!_PyType_HasFeature(Py_TYPE(descr), Py_TPFLAGS_METHOD_DESCRIPTOR), LOAD_ATTR);
            STAT_INC(LOAD_ATTR, hit);
        ...
        inst(LOAD_ATTR_METHOD_NO_DICT, (unused/1, type_version/2, index/1, self -- res2 if (oparg & 1), res)) {
            assert(cframe.use_tracing == 0);
            PyTypeObject *self_cls = Py_TYPE(self);
            ...
            assert(_PyType_HasFeature(Py_TYPE(entry->value), Py_TPFLAGS_METHOD_DESCRIPTOR));
            res2 = Py_NewRef(entry->value);
            res = self;
            assert(oparg & 1);
        }

        inst(LOAD_ATTR_METHOD_LAZY_DICT, (unused/1, type_version/2, index/1, self -- res2 if (oparg & 1), res)) {
            assert(cframe.use_tracing == 0);
            PyTypeObject *self_cls = Py_TYPE(self);
            DEOPT_IF(self_cls->tp_version_tag != type_version, LOAD_ATTR);
