...
"     flags, codestring, constants, varnames, filename, name, qualname,\n"
...
code_new_impl(PyTypeObject *type, int argcount, int posonlyargcount,
              ...
              PyObject *code, PyObject *consts, PyObject *varnames,
...
code_new(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    varnames = PyTuple_GET_ITEM(args, 8);
    if (!PyUnicode_Check(PyTuple_GET_ITEM(args, 9))) {
        _PyArg_BadArgument("code", "argument 10", "str", PyTuple_GET_ITEM(args, 9));
        goto exit;
    }
    if (PyUnicode_READY(PyTuple_GET_ITEM(args, 9)) == -1) {
        goto exit;
    }
    filename = PyTuple_GET_ITEM(args, 9);
    ...
    qualname = PyTuple_GET_ITEM(args, 11);
...
skip_optional:
    return_value = code_new_impl(type, argcount, posonlyargcount, kwonlyargcount, nlocals, stacksize, flags, code, consts, varnames, filename, name, qualname, firstlineno, linetable, exceptiontable, freevars, cellvars);
...
"        co_filename=None, co_name=None, co_qualname=None,\n"
...
code_replace_impl(PyCodeObject *self, int co_argcount,
                  ...
                  PyObject *co_consts, PyObject *co_varnames,
                  PyObject *co_freevars, PyObject *co_cellvars,
                  PyObject *co_filename, PyObject *co_name,
                  PyObject *co_qualname, PyBytesObject *co_linetable,
...
code_replace(PyCodeObject *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    static struct {
        ...
        .ob_item = { &_Py_ID(co_argcount), &_Py_ID(co_posonlyargcount), &_Py_ID(co_kwonlyargcount), &_Py_ID(co_nlocals), &_Py_ID(co_stacksize), &_Py_ID(co_flags), &_Py_ID(co_firstlineno), &_Py_ID(co_code), &_Py_ID(co_consts), &_Py_ID(co_varnames), &_Py_ID(co_freevars), &_Py_ID(co_cellvars), &_Py_ID(co_filename), &_Py_ID(co_name), &_Py_ID(co_qualname), &_Py_ID(co_linetable), &_Py_ID(co_exceptiontable), },
    };
    #undef NUM_KEYWORDS
    ...
    static const char * const _keywords[] = {"co_argcount", "co_posonlyargcount", "co_kwonlyargcount", "co_nlocals", "co_stacksize", "co_flags", "co_firstlineno", "co_code", "co_consts", "co_varnames", "co_freevars", "co_cellvars", "co_filename", "co_name", "co_qualname", "co_linetable", "co_exceptiontable", NULL};
    ...
    if (args[9]) {
        if (!PyTuple_Check(args[9])) {
            _PyArg_BadArgument("replace", "argument 'co_varnames'", "tuple", args[9]);
            goto exit;
        }
        co_varnames = args[9];
        if (!--noptargs) {
    ...
    if (args[10]) {
        if (!PyTuple_Check(args[10])) {
            _PyArg_BadArgument("replace", "argument 'co_freevars'", "tuple", args[10]);
            goto exit;
        }
        co_freevars = args[10];
        if (!--noptargs) {
    ...
    if (args[11]) {
        if (!PyTuple_Check(args[11])) {
            _PyArg_BadArgument("replace", "argument 'co_cellvars'", "tuple", args[11]);
            goto exit;
        }
        co_cellvars = args[11];
        if (!--noptargs) {
    ...
    if (args[12]) {
        if (!PyUnicode_Check(args[12])) {
            _PyArg_BadArgument("replace", "argument 'co_filename'", "str", args[12]);
            goto exit;
        }
        if (PyUnicode_READY(args[12]) == -1) {
            goto exit;
        }
        co_filename = args[12];
        if (!--noptargs) {
    ...
    if (args[13]) {
        if (!PyUnicode_Check(args[13])) {
            _PyArg_BadArgument("replace", "argument 'co_name'", "str", args[13]);
            goto exit;
        }
    ...
    if (args[14]) {
        if (!PyUnicode_Check(args[14])) {
            _PyArg_BadArgument("replace", "argument 'co_qualname'", "str", args[14]);
            goto exit;
        }
...
skip_optional_kwonly:
    return_value = code_replace_impl(self, co_argcount, co_posonlyargcount, co_kwonlyargcount, co_nlocals, co_stacksize, co_flags, co_firstlineno, co_code, co_consts, co_varnames, co_freevars, co_cellvars, co_filename, co_name, co_qualname, co_linetable, co_exceptiontable);
...
/*[clinic end generated code: output=85b9130a82500c49 input=a9049054013a1b77]*/
