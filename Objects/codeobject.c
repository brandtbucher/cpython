...
static const char assert0_compressed[3] = {
...
PyCode_NewEmpty(const char *filename, const char *funcname, int firstlineno)
{
    ...
    code_compressed = PyBytes_FromStringAndSize(assert0_compressed, 3);
    if (code_compressed == NULL) {
        goto failed;
    }
...
code_dealloc(PyCodeObject *co)
{
    ...
    Py_XDECREF(co->co_code_compressed);
    if (_Py_OPCODE(_PyCode_CODE(co)[0]) != EXPAND) {
...
{
    if (_Py_OPCODE(_PyCode_CODE(code)[0]) == EXPAND) {
