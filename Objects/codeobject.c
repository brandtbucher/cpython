...
code_getcodeadaptive(PyCodeObject *code, void *closure)
{
    ...
    int size = 0;
    for (int i = 0; i < (int)(PyBytes_GET_SIZE(code->co_code) / sizeof(_Py_CODEUNIT)); i++) {
        size += 1 + _PyOpcode_Caches[_PyOpcode_Adaptive[_Py_OPCODE(((_Py_CODEUNIT *)PyBytes_AS_STRING(code->co_code))[i])]];
    }
    return PyBytes_FromStringAndSize((char *)code->co_first_instr, size * sizeof(_Py_CODEUNIT));
}
...
/*[clinic end generated code: output=b6cd9988391d5711 input=08940b805f1d6e69]*/
{
