...
class Compiler:
    ...
    def _use_ghccc(self, ll: pathlib.Path) -> None:
        if self._ghccc:
            before = ll.read_text()
            after = re.sub(
                r"((?:noalias |nonnull )*(?:ptr|%struct._PyInterpreterFrame\*) @_JIT_(?:CONTINUE|DEOPT|ENTRY|ERROR|JUMP))",
                r"ghccc \1",
                before,
            )
    ...
    async def _compile(self, opname, oparg_low_bit, stack_level, c, tempdir) -> None:
        defines = [f"-D_JIT_OPCODE={opname}", f"-DOPARG_LOW_BIT={oparg_low_bit}", f"-D_JIT_STACK_LEVEL={stack_level}"]
        ll = pathlib.Path(tempdir, f"{opname}_{oparg_low_bit}_{stack_level}.ll").resolve()
        o = pathlib.Path(tempdir, f"{opname}_{oparg_low_bit}_{stack_level}.o").resolve()
        await run(self._clang, *CFLAGS, "-emit-llvm", "-S", *defines, "-o", ll, c)
        self._use_ghccc(ll)
        await run(self._clang, *CFLAGS, "-c", "-o", o, ll)
        self._stencils_built[opname, oparg_low_bit, stack_level] = await self._parser(
    ...
    async def build(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            await asyncio.gather(
                ...
                *[
                    self._compile(opname, oparg_low_bit, stack_level, TOOLS_JIT_TEMPLATE, tempdir)
                    for opname in sorted(re.findall(r"\n {8}case (\w+): \{\n", generated_cases))
                    for stack_level in range(MAX_STACK_LEVEL + 1)
                    for oparg_low_bit in (0, 1)
                ],
            )
    ...
    def dump(self) -> str:
        ...
        for (opname, oparg_low_bit, stack_level), stencil in sorted(self._stencils_built.items()):
            if not stencil.body:
                continue
            opnames.setdefault(opname, set()).add((oparg_low_bit, stack_level))
            lines.append(f"// {opname}(oparg & 1 == {oparg_low_bit}) @ {stack_level}")
            for line in stencil.disassembly:
                lines.append(f"// {line}")
            body = ",".join(f"0x{byte:02x}" for byte in stencil.body)
            lines.append(
                f"static const unsigned char {opname}_{oparg_low_bit}_{stack_level}_stencil_bytes[{len(stencil.body)}] = {{{body}}};"
            )
            holes = []
            loads = []
            for hole in stencil.holes:
            ...
            lines.append(
                f"static const Hole {opname}_{oparg_low_bit}_{stack_level}_stencil_holes[{len(holes) + 1}] = {{"
            )
            for hole in holes:
                lines.append(hole)
            lines.append(
            ...
            lines.append(f"}};")
            lines.append(
                f"static const SymbolLoad {opname}_{oparg_low_bit}_{stack_level}_stencil_loads[{len(loads) + 1}] = {{"
            )
            for load in loads:
                lines.append(load)
            lines.append(
        ...
        lines.append(f"")
        lines.append(f"#define INIT_STENCIL(OPCODE, OPARG_LOW_BIT, STACK_LEVEL) {{                                  \\")
        lines.append(f"    .nbytes = Py_ARRAY_LENGTH(OPCODE##_##OPARG_LOW_BIT##_##STACK_LEVEL##_stencil_bytes),     \\")
        lines.append(f"    .bytes = OPCODE##_##OPARG_LOW_BIT##_##STACK_LEVEL##_stencil_bytes,                       \\")
        lines.append(f"    .nholes = Py_ARRAY_LENGTH(OPCODE##_##OPARG_LOW_BIT##_##STACK_LEVEL##_stencil_holes) - 1, \\")
        lines.append(f"    .holes = OPCODE##_##OPARG_LOW_BIT##_##STACK_LEVEL##_stencil_holes,                       \\")
        lines.append(f"    .nloads = Py_ARRAY_LENGTH(OPCODE##_##OPARG_LOW_BIT##_##STACK_LEVEL##_stencil_loads) - 1, \\")
        lines.append(f"    .loads = OPCODE##_##OPARG_LOW_BIT##_##STACK_LEVEL##_stencil_loads,                       \\")
        lines.append(f"}}")
        lines.append(f"")
        lines.append(f"static const Stencil deopt_stencils[2][MAX_STACK_LEVEL + 1] = {{")
        for oparg_low_bit, stack_level in sorted(opnames["deopt"]):
            lines.append(f"        [{oparg_low_bit}][{stack_level}] = INIT_STENCIL(deopt, {oparg_low_bit}, {stack_level}),")
        lines.append(f"}};")
        lines.append(f"")
        lines.append(f"static const Stencil error_stencils[2][MAX_STACK_LEVEL + 1] = {{")
        for oparg_low_bit, stack_level in sorted(opnames["error"]):
            lines.append(f"        [{oparg_low_bit}][{stack_level}] = INIT_STENCIL(error, {oparg_low_bit}, {stack_level}),")
        lines.append(f"}};")
        lines.append(f"")
        lines.append(f"static const Stencil trampoline_stencils[2][MAX_STACK_LEVEL + 1] = {{")
        for oparg_low_bit, stack_level in sorted(opnames["trampoline"]):
            lines.append(f"        [{oparg_low_bit}][{stack_level}] = INIT_STENCIL(trampoline, {oparg_low_bit}, {stack_level}),")
        lines.append(f"}};")
        lines.append(f"")
        lines.append(f"static const Stencil stencils[512][2][MAX_STACK_LEVEL + 1] = {{")
        del opnames["deopt"], opnames["error"], opnames["trampoline"]
        for opname, oparg_low_bits_and_stack_levels in sorted(opnames.items()):
            for oparg_low_bit, stack_level in sorted(oparg_low_bits_and_stack_levels):
                lines.append(f"    [{opname}][{oparg_low_bit}][{stack_level}] = INIT_STENCIL({opname}, {oparg_low_bit}, {stack_level}),")
        lines.append(f"}};")
        lines.append(f"")
