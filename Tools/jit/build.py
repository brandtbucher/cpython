...
class Compiler:
    ...
    async def _compile(self, opname, stack_level, c) -> None:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            async with self._semaphore:
                ...
                if process.returncode:
                    raise RuntimeError(f"{self._clang} exited with {process.returncode}")
                self._stencils_built[opname, stack_level] = await ObjectParserDefault(o, self._readobj, self._objdump).parse()
    ...
    def dump(self) -> str:
        ...
        for (opname, stack_level), stencil in sorted(self._stencils_built.items()):
            ...
            lines.append(f'static const unsigned char {opname}_{stack_level}_stencil_bytes[{len(stencil.body)}] = {{{body}}};')
            holes = []
            loads = []
            for hole in stencil.holes:
            ...
            lines.append(f"static const Hole {opname}_{stack_level}_stencil_holes[{len(holes) + 1}] = {{")
            for hole in holes:
                lines.append(hole)
            lines.append(f"    {{.kind =            0, .offset = 0x000, .addend =    0, .value = 0}},")
            lines.append(f"}};")
            lines.append(f"static const SymbolLoad {opname}_{stack_level}_stencil_loads[{len(loads) + 1}] = {{")
            for  load in loads:
                lines.append(load)
